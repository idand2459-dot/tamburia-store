...
client/
  src/
    ...
    js/          all React components
    css/         stylesheets, split by concern (see below)
  build/         production bundle, not committed

test/            end-to-end test suites
scripts/         CSV product import, demo-data seeding
uploads/         admin image uploads — contents are gitignored
legacy/          archived first prototype, not served
```

Stylesheets are modular rather than one large file:

```
client/src/css/
  app.css                 the only entry point — imports everything below,
                          in the order that defines the cascade
  base/                   design tokens, reset, typography
  layout/                 grid, header, footer
  components/             buttons, modals, notifications, loading states
  features/               one file per feature: catalog, cart, admin,
                          calculator, orders, reviews, wishlist, pages…
  features/_category-themes.css   per-category theming — imported last,
                          because it repaints components above it
```

Import order in `app.css` is load-bearing: a dark-mode or breakpoint rule has to
sit in the same partial as the rule it overrides, or it stops winning.
...
| `npm run seed:demo` | Inserts demo orders and reviews, for screenshots or a walkthrough |
| `npm run seed:demo:clear` | Removes exactly that demo data again |

Demo data is deliberately separate from the migrations and never runs on startup —
the migration runner only reads `.sql` files from `server/db/migrations`. Re-running
the seed replaces its own rows rather than duplicating them, and `--clear` removes
only what it inserted, so the database can be handed over clean.
